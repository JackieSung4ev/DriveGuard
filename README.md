...
DriveGuard 是一个面向 Debian/Ubuntu 服务器的独立 Google Drive 备份脚本。它通过中文交互菜单配置 Google Drive 授权、网站目录备份、MySQL/MariaDB 数据库备份、加密、定时任务、日志、保留份数清理和 cron 守护。
...
- Google Drive 远程目录，默认 `driveguard`
- 每个网站/数据库保留份数，默认 `7`
- 本地备份暂存目录
- cron 定时表达式，默认 `0 3 * * *`
...
- `上传失败`：检查 `/var/log/driveguard/rclone.log` 和服务器到 Google 的网络连通性。
