...
### 备份到默认的 `driveguard` 文件夹

Google Drive 目录有两种常见做法。

第一种更简单：`root_folder_id` 留空，在 `sudo dg configure` 里把 `云端远程目录` 保持为 `driveguard`。这样 rclone remote 仍指向整个 Google Drive，不限制 rclone 的可见范围，但 DriveGuard 只会把备份写到：
...
如果 `driveguard` 文件夹不存在，DriveGuard 上传前会通过 `rclone mkdir` 自动创建。

第二种更收敛：让 rclone remote 的根目录就是某个专用父文件夹，例如 `backup`。先在 Google Drive 网页新建这个父文件夹，打开后从浏览器地址栏复制最后那段文件夹 ID，例如：
...
如果使用了专用父文件夹，并希望 DriveGuard 文件直接放在该父文件夹下的 `site/` 和 `database/`，则把 DriveGuard 的 `云端远程目录` 留空。
